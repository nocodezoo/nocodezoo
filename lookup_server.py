...
import http.server, socketserver, json, re, html as htmlmod, urllib.request
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            def strip_tags(s):
                def repl(em):
                    n = em.group(1); c = htmlmod.entities.get(n, 0)
                    return chr(c) if isinstance(c, int) else em.group(0)
                s = re.sub("<[^>]+>", "", s)
                return re.sub(r"&(\w+?);", repl, s).strip()
